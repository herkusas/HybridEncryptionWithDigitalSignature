...
namespace HybridEncryptionWithDigitalSignature
{
    public class HybridEncryption
    {
        ...
        public EncryptedPacket EncryptData(byte[] original, RsaWithRsaParameterKey rsaParams, DigitalSignature digitalSignature)
        {
            ...
            using (var hmac = new HMACSHA256(decryptedSessionKey))
            {
                ...
                if (!Compare(encryptedPacket.Hmac, hmacToCheck))
                {
                    throw new CryptographicException(
                        "HMAC for decryption does not match encrypted package");
                }
